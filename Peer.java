...
public class Peer extends Thread implements Serializable {
	...
	public void view() {
		...
		System.out.println("Coordinates: " + this.x1 + " " + this.y1 + " "
				+ this.x2 + " " + this.y2);
	...
	public void viewCoordinates() {
		System.out.println("Coordinates of " + this.getNodeIdentifier() + ":"
	...
	public void copyNeighbors(Peer n) {
		for (Peer i : n.neighbors) {
			...
			if (!this.getNodeIdentifier().equals(i.getNodeIdentifier())) {
				Boolean flag = false;
				// You need to compare node identifier, because object can be
				// changed which passing
	...
	/*
	 * Function to check 2 peers are neighbor or not It takes coordinates of 2
	 * peers/nodes Based on coordinates of peers, creates 4 edges of each peer
	 * and checks any one of 4 edge of FIRST peer overlaps Any one of 4 Edges of
	 * SECOND peer
	 */
	public boolean isNeighbor(Peer p) {
		...
		if ((x1 < x11 && x2 < x11) || (x1 > x22 && x2 > x22)
				|| (y1 < y11 && y2 < y11) || (y1 > y22 && y2 > y22)) {
			return false;
		}
	...
	public void spiltMySpace(Peer newPeer, int x, int y) {
		...
		if ((x2 - x1) <= (y2 - y1)) { // square or length is less than height
			/* divide horizontally */
			// Check y coordinate of given point(x,y) lies in which region i.e.
			// upper or lower
			// If it lies in upper, then new Peer will be upper half divided
			// node. lower will be entry point or original node.
			// If it lies lower, Then new Peer will be lower half divided node.
			// Upper will be entrypoint or original node
			if (y < ((y1 + y2) / 2)) {
			...
			/* divide vertically */
			if (x < ((x1 + x2) / 2)) { // check x lies in which region, right or
										// left
	...
	/*
	 * When you add new peer to current peer, the neighbors of current peer's
	 * neighbors also change. for ex. if there are p1 and p2 and if you add p3,
	 * it will also become a neighbor of p2 check this
	 */
	public void changeNeighborOfCurrentPeerNeighbors(Peer newPeer) {
		...
		for (Peer temp : this.getNeighbors()) {
			if (temp.isNeighbor(newPeer)
					&& !(temp.getNodeIdentifier().equals(newPeer
							.getNodeIdentifier()))) {
				...
				if (!((temp.getNeighbors()).contains(newPeer))) {
					/*
					 * Create socket to that Peer and add current peer to it's
					 * neighbor list
					 */
					...
					try {
						...
						System.out
								.println(" Socket to Entry Peer created.. TO CHANGE NEIGHBOR ");
						OutputStream peerOs1 = peerSocket1.getOutputStream();
						ObjectOutputStream peerOos1 = new ObjectOutputStream(
								peerOs1);

						InputStream peerIs1 = peerSocket1.getInputStream();
						ObjectInputStream peerOis1 = new ObjectInputStream(
								peerIs1);
		...
		for (Peer newTemp : this.getNeighbors()) {
			...
			if (!(newTemp.getNodeIdentifier()).equals(newPeer
					.getNodeIdentifier())) {
				/*
				 * Create socket to that Peer and remove current peer from it's
				 * neighbor list
				 */
				...
				try {
					...
					ObjectOutputStream peerOos2 = new ObjectOutputStream(
							peerOs2);
	...
	public BigInteger firstHashValue(String keyword) {
		try {
			...
			messageDigest.update(keyword.getBytes(), 0, keyword.length());
	...
	public static void main(String args[]) {
		...
		try {
			...
			System.out
					.println("Enter Any One Command : 1. join  2.insert 3. serach.");
			while ((userInput = stdIn.readLine()) != null) {
				String[] in = userInput.split("\\s+");
				if (in[0].equals("join")) {
					System.out
							.println("Enter command, hostname , node, portnumber name, x coordinate, y coordinate separated by space : ");
					...
					/*
					 * Check for duplicate node identifier , HashMap can be used
					 * to check
					 */
				...
				oos.writeObject(p); // Send object to bootstrap server for entry
									// point peer request

				EntryPoint = (Peer) ois.readObject(); // Server returns the
														// entry point peer
				System.out.println("ENTRY POINT: " + EntryPoint);
				EntryPoint.view();

				if (EntryPoint.getNodeIdentifier()
						.equals(p.getNodeIdentifier())) {
					p = EntryPoint;
				}
				...
				try (ServerSocket server = new ServerSocket(peerPortNumber)) {
					...
					if (p.getCommand().equals("insert")
							|| p.getCommand().equals("search")) {
						System.out.println(" Routing Path :"
								+ EntryPoint.getNodeIdentifier());
					}

					if (!EntryPoint.getNodeIdentifier().equals(
							p.getNodeIdentifier())) {
						...
						ObjectOutputStream peerOos = new ObjectOutputStream(
								peerOs);

						InputStream peerIs = peerSocket.getInputStream();
						ObjectInputStream peerOis = new ObjectInputStream(
								peerIs);
						...
						while (!(temp.getNodeIdentifier().equals(p
								.getNodeIdentifier()))) {
							int NewPeerPort = temp.getPortnumber();
							String NewPeerIp = temp.getIP();
							Socket NewPeerSocket = new Socket(NewPeerIp,
									NewPeerPort);
							OutputStream NewPeerOs = NewPeerSocket
									.getOutputStream();
							ObjectOutputStream NewPeerOos = new ObjectOutputStream(
									NewPeerOs);

							InputStream NewPeerIs = NewPeerSocket
									.getInputStream();
							ObjectInputStream NewPeerOis = new ObjectInputStream(
									NewPeerIs);
					...
					System.out.println(EntryPoint.getNodeIdentifier()
